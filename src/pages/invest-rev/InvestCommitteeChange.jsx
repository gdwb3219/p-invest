...
function InvestCommitteeChange() {
  ...
  const confirmStage = useCallback(() => {
    ...
    closeModal();
  }, [modalRowIndex, modalRow, editColumn, valueAfter, reason, closeModal]);

  const removeStaged = useCallback((targetId) => {
    setStagedMap((prev) => {
      const next = new Map(prev);
                  ...
                  {stagedMap.size === 0 ? (
                    ...
                    <div className="invest-rev-request__table-wrap">
                      <table className="invest-rev-request__table">
                        ...
                        <tbody>
                          {Array.from(stagedMap.entries()).map(
                            ([id, entry]) => (
                              <tr key={id}>
                                ...
                                <td>
                                  {displayCell(
                                    entry.afterData?.[entry.editColumn],
                                  )}
                                </td>
                                <td
                                  style={{
